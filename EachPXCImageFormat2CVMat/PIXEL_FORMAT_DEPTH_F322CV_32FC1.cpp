//#define ENABLE_PIXEL_FORMAT_DEPTH_F322CV_32FC1_CPP
#ifdef ENABLE_PIXEL_FORMAT_DEPTH_F322CV_32FC1_CPP
...
bool PIXEL_FORMAT_DEPTH_F322CV_32FC1(PXCImage *pxcImage, cv::Mat &image)
{
	//获取图像的格式
	PXCImage::PixelFormat format = PXCImage::PIXEL_FORMAT_DEPTH_F32;
	...
	//转换为OpenCV格式的图像
	image = cv::Mat(cv::Size(width, height), CV_32FC1, data.planes[0]);
	
	pxcImage->ReleaseAccess(&data);
#ifdef DEBUG
	std::cout << "[+] succe:PIXEL_FORMAT_DEPTH convert to CV_32FC1 successful." << std::endl;
#endif
	return true;
}
...
int wmain(int argc, WCHAR* argv[]) {
	...
	while(keepRunning){
		...
		if(PIXEL_FORMAT_DEPTH_F322CV_32FC1(sample->depth, image) != true){
#ifdef DEBUG
			std::cout << "[-] Error PIXEL_FORMAT_DEPTH_F32 convert to CV_32FC1 processing failed." << std::endl;
#endif
