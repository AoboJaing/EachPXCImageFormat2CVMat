#define ENABLE_PIXEL_FORMAT_DEPTH_CONFIDENCE2CV_8UC1_CPP
#ifdef ENABLE_PIXEL_FORMAT_DEPTH_CONFIDENCE2CV_8UC1_CPP
...
bool PIXEL_FORMAT_DEPTH_CONFIDENCE2CV_8UC1(PXCImage *pxcImage, cv::Mat &image)
{
	//获取图像的格式
	PXCImage::PixelFormat format = PXCImage::PIXEL_FORMAT_DEPTH_CONFIDENCE;
	...
	pxcImage->ReleaseAccess(&data);
#ifdef DEBUG
	std::cout << "[+] succe:PIXEL_FORMAT_DEPTH_CONFIDENCE convert to CV_8UC1 successful." << std::endl;
#endif
	return true;
}
...
int wmain(int argc, WCHAR* argv[]) {
	...
	pxcSenseManager->EnableStream(PXCCapture::STREAM_TYPE_DEPTH, image_size.width, image_size.height, 30, PXCCapture::Device::STREAM_OPTION_DEPTH_CONFIDENCE);
	...
	while(keepRunning){
		...
		if(PIXEL_FORMAT_DEPTH_CONFIDENCE2CV_8UC1(sample->depth, image) != true){
#ifdef DEBUG
			std::cout << "[-] Error PIXEL_FORMAT_DEPTH_CONFIDENCE convert to CV_8UC1 processing failed." << std::endl;
#endif
