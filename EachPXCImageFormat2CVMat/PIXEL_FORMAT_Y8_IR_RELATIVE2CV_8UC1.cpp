//#define ENABLE_PIXEL_FORMAT_Y8_IR_RELATIVE2CV_8UC1_CPP
#ifdef ENABLE_PIXEL_FORMAT_Y8_IR_RELATIVE2CV_8UC1_CPP
...
#define ENABLE_CV_SHOW
 ...
 //相对红外线
bool PIXEL_FORMAT_Y8_IR_RELATIVE2CV_8UC1(PXCImage *pxcImage, cv::Mat &image)
{
	//获取图像的格式
	...
	pxcImage->ReleaseAccess(&data);
#ifdef DEBUG
	std::cout << "[+] succe:PIXEL_FORMAT_Y8_IR_RELATIVE convert to CV_8UC1 successful." << std::endl;
#endif
	return true;
}
...
int wmain(int argc, WCHAR* argv[]) {
	...
	while(keepRunning){
		...
		if(PIXEL_FORMAT_Y8_IR_RELATIVE2CV_8UC1(sample->ir, image) != true){
#ifdef DEBUG
			std::cout << "[-] Error PIXEL_FORMAT_Y8_IR_RELATIVE convert to CV_8UC1 processing failed." << std::endl;
#endif
